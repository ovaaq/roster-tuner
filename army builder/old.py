            ...
            for enhancement in faction[2]:
                ...
                for keyword in enhancement[4]:
                    if keyword in keywords:
                        add = True
                if add and (army_list["size"] - army_list["points"] >= enhancement[1]) and (
                        enhancement[0] not in army_list.get("enhancements")):
...
def get_selected_list(selection_type):
    ...
    for unit_type in loop_list:
        for index, item in enumerate(army_list.get(unit_type)):
            ...
            if selection_type == "delete" or (selection_type == "enhancements" and not is_epic_hero(item[0]) and len(
                    army_list.get("enhancements")) < 1 and not "enhancement:%" in item[
                2] and index not in cannot_enhancement) or (
                    selection_type == "warlord" and item[0] not in cannot_warlord):
...
def get_units_with_values(faction_name, max_cost):
    ...
    for item in data.units:
        ...
        if (item[9][0] in faction_names and item[0] not in army_list.get("epic heroes") and item[1] <= int(
                max_cost) and same_character_count < 2 and same_battleline_count < 6):
...
"""
    ...
    return """""" + army_list.get("name") + """ (""" + str(army_list.get("points")) + """/""" + str(
        army_list.get("size")) + """ Points) |
--- |
